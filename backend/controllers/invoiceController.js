  ...
  try {
    ...
    if (roomBookingId) {
      await RoomBooking.findOneAndUpdate(
        { _id: roomBookingId, hotelId: req.user.hotelId }, 
...
const getCombinedStats = async (start, end, source, hotelId) => {
  let total = 0, count = 0;
  
  if (source === 'restaurant' || source === 'all') {
    const invData = await Invoice.aggregate([
      { $match: { createdAt: { $gte: start, $lte: end }, hotelId: hotelId } },
  ...
  if (source === 'room' || source === 'all') {
    const roomData = await RoomBooking.aggregate([
      { $match: { createdAt: { $gte: start, $lte: end }, status: 'Checked-Out', hotelId: hotelId } },
...
const getInvoiceStats = async (req, res) => {
  try {
    ...
    const todayStats = await getCombinedStats(todayRange.start, todayRange.end, source, hotelId);

    // 2. Month's Billing
    const monthRange = getDateRange("month");
    const monthStats = await getCombinedStats(monthRange.start, monthRange.end, source, hotelId);

    // 3. Yearly Billing
    const yearRange = getDateRange("year");
    const yearStats = await getCombinedStats(yearRange.start, yearRange.end, source, hotelId);
    ...
    if (chartType === "day") {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const dayStats = await getCombinedStats(start, end, source, hotelId);
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        const weekStats = await getCombinedStats(dStart, dEnd, source, hotelId);
      ...
      for (let y = currentYear - 4; y <= currentYear; y++) {
        ...
        const yearStats = await getCombinedStats(start, end, source, hotelId);
      ...
      for (let m = 0; m < 12; m++) {
        ...
        const mStats = await getCombinedStats(mStart, mEnd, source, hotelId);
